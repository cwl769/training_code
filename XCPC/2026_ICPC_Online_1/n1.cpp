...
int main()
{
    ...
    std::vector<std::vector<i64>> svx(n * 2 + 2), svy(n * 2 + 2);
    for (int i = 1; i <= n; ++i)
    {
        int x = py[i];
        int y = pb[i];
        ...
        svx[x + n].emplace_back(-inf);
        svy[y + n].emplace_back(-inf);
    }
    ...
    for (int j = 1; j <= n; ++j)
    {
        ...
        if (1)
        {
            ...
            {
                if (svx[x + n][it - bx[x + n].begin()] >= f[j])
                    break;
                svx[x + n][it - bx[x + n].begin()] = f[j];
                int i = it->second;
                f[i] = std::max(f[i], f[j]);
            }
        }
        if (1)
        {
            ...
            {
                if (svy[y + n][it - by[y + n].begin()] >= f[j])
                    break;
                svy[y + n][it - by[y + n].begin()] = f[j];
                int i = it->second;
                f[i] = std::max(f[i], f[j]);
            }
        }
